...
| `alpaca_kit/mcp/` | the read-only MCP tool surface; order tools live in `account.py` and register only under `ALPACA_KIT_ENABLE_ORDERS=1`; screen/breadth results disk-cache under `data/.screen_cache` (`mcp/cache.py`: keyed bed+code+day+kind, never inside a bed) |
...
| `face/` | the operator's chat face: a Node 22 process hosting dsh in-process from profile `face` (bundles dsh-base only), serving chat-light + the read-only `/market`/`/account` instruments at 127.0.0.1:3090; `strategies/*` are operator-rostered dsh-workspace channels; Gate-2 surface — run/instrument/drill docs in `face/README.md` |
