...
> **2026-07-14 — LLM STACK SWITCH SHIPPED (branch `feat/llm-stack-switch`; 1946 tests):** a
> per-entity (Sonia/Kairos) Claude↔DeepSeek switch on the :8100 Models page, backed by the shared
> `state/llm_stack.json` (`ALPHA_LLM_STACK_FILE`) read by `make_client` per call — precedence env >
> file > defaults, resolved per-field. The switch is instant mid-session, also steers CLI batch runs,
> and is restart-persistent; named stacks + role→entity map live in `alpha/llm/stack.py`. Spec:
> `docs/superpowers/specs/2026-07-14-llm-stack-switch-design.md`.

> **2026-07-16 — BODY SIX-COMPONENT COMPLETION SHIPPED (branch `feat/body-six-components`, 2046
> tests):** `H` grows from `(p, K, M)` to `(p, K, M, C, W, A)` — connector/workflow/subagent join
> doctrine/skills/memory as real, gated `HarnessState` components, built in one arc via one shared
> integration pattern stamped three times: registry field on `HarnessState` (additive, empty-default,
> legacy-dump compatible) → brain.json serialization → `EditRecord` target_kinds + `try_apply_op`
> waist ops (`write_connector`/`patch_connector`/`disable_connector`,
> `write_workflow`/`patch_workflow`/`retire_workflow`,
> `write_subagent`/`patch_subagent`/`retire_subagent`, no-delete, same provenance/governance as
> p/K/M) → `extract_ops` vocabulary → `render_brain_summary` budgeted index → three real
> `alpha_web` pages (`/connector` `/workflow` `/subagent`, replacing the Task-8-deleted
> `brain_stub`/`.brain-stub` placeholders) → one seed (`alpaca` connector; W/A seed empty, grown
> through teaching). Waist lints: `impl_ref` resolves in the live registry; `env_keys` refuses
> value-shaped entries; `instructions`/`description` char-caps; workflow `steps[].ref` referential
> integrity; subagent `tools` is a lint-enforced subset of the parent-registerable tool names. **This
> arc is declarative-only by design** — entries are stored/editable/evolvable/prompt-visible, but
> the workflow EXECUTOR (engine stepping through steps), subagent DISPATCH (the `G` pass), and a
> hooks/cron analog (deterministic lifecycle triggers beyond today's pre-action L4-veto/
> `ActivityPolicy`-tier/sizing-decorator layer) are explicitly deferred to a later arc (US-2/R3
> boundary); `PASS_TOOLS["G"]` stays a reserved empty pass and `PASS_ORDER` stays `(p, G, K, M)` —
> the new C/W/A ops ride `PASS_TOOLS` (waist dispatch), never `PASS_ORDER`, so the self-study
> Refiner never auto-proposes Body edits (teaching/`user_direct` only, same governance as p/K/M).
> Second half of the arc gave **Sonia a bounded T0 observe loop**: `SoniaAgent.respond` now routes
> through the `run_conversation` dispatch seam under a fail-closed `ActivityPolicy`, exposing six
> `view_*` brain-browse tools (doctrine/skill/lesson/workflow/connector/subagent) plus
> `search_episodes` — two-tier progressive disclosure (an always-in-prompt budgeted index, full
> detail on demand) — and three **connector-gated market tools**, `market_snapshot`/`daily_bars`/
> `latest_decisions`, registered only when the seeded `alpaca` connector entry is `enabled` and its
> `env_keys` are present in the environment (else simply absent, never a boot error); when present
> they wrap a lazily-built RAW source in `GuardedSource(AsOfGuard(today))` per call and fail soft
> (`{"ok": False, "error": ...}`, never raise into the loop). `decide()` and `extract_ops` keep their
> full always-in-prompt render (verdict symmetry untouched); the market/view tools apply to the
> conversational face only. Spec `docs/superpowers/specs/2026-07-16-body-six-components-design.md`,
> plan `docs/superpowers/plans/2026-07-16-body-six-components.md`.
