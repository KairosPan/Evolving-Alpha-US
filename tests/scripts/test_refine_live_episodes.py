...
def _kw(tmp_path, brain_dir, conflicts_dir, **extra):
    return dict(
        brain_dir=str(brain_dir), conflicts_dir=str(conflicts_dir),
        agent_llm_factory=lambda: MockLLMClient("{}"),
        refiner_llm_factory=lambda: MockLLMClient(_REFINER_SCRIPT),
        agent_factory=lambda h: _PickRun(), horizon=2,
        loop_config=LoopConfig(horizon=2, screen=False, size=False),
        proposals_root=str(tmp_path / "proposals"), **extra)


def test_autonomous_mode_writes_episodes_when_db_given(tmp_path, monkeypatch):
    """Episode write-through at the credit seam is AUTONOMOUS-mode behavior (the pre-pivot
    in-place run). Charter conformance 2026-07-09 moved the default to propose mode."""
    ...
    rl.run_refine_live(src, cal[0], cal[-1],
                       **_kw(tmp_path, brain_dir, conflicts_dir, episodes_db=str(db),
                             mode="autonomous"))

    s = EpisodeStore.open(str(db))
    assert len(s.all()) > 0, "Expected episodes to be written to brain.db in autonomous mode"


def test_propose_mode_writes_no_live_episodes(tmp_path):
    """Charter conformance (2026-07-09 review blocker): a discarded fork must die with its
    session — the default propose run reads recall/taboo from the live DB but writes NO episode
    to it. The episodes DB is byte-absent/empty after the run."""
    brain_dir = tmp_path / "brain"
    conflicts_dir = tmp_path / "conflicts"
    db = tmp_path / "brain.db"
    ...
    out = rl.run_refine_live(src, cal[0], cal[-1],
                             **_kw(tmp_path, brain_dir, conflicts_dir, episodes_db=str(db)))

    assert out["mode"] == "propose"
    assert len(EpisodeStore.open(str(db)).all()) == 0, \
        "propose mode must not write live episodes (the fork's evidence dies with its session)"
...
def test_refine_live_none_db_still_works(tmp_path):
    ...
    out = rl.run_refine_live(src, cal[0], cal[-1],
                             **_kw(tmp_path, brain_dir, conflicts_dir, episodes_db=None))
...
def test_refine_live_recalls_own_brain_and_vetoes_taboo(tmp_path, monkeypatch):
    """Seed the brain so RUN is taboo; with the read ON the loop recalls it -> vetoes RUN before
    entry -> writes NO new RUN episode (count stays at the 3 seeds). Run in AUTONOMOUS mode so
    episode write-through is live — in propose mode nothing writes, which would make this
    assertion vacuous (the propose-mode recall read path shares the same recall_store wiring)."""
    from datetime import date
    monkeypatch.setenv("ALPHA_UNSAFE_AUTONOMOUS", "1")
    brain_dir = tmp_path / "brain"
    conflicts_dir = tmp_path / "conflicts"
    ...
    rl.run_refine_live(
        ...
        **{**_kw(tmp_path, brain_dir, conflicts_dir, episodes_db=db, mode="autonomous"),
